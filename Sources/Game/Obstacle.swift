...
final class Obstacle {
    ...
    private func build(customLength: Float?) {
        ...
        case .lowBarrier:
            ...
            halfW = 1.0; minY = 0; maxY = 0.85; length = 0.4
        ...
        case .poleBarrier:
            ...
            halfW = 1.0; minY = 0; maxY = 0.8; length = 0.3
        ...
        case .highBarrier:
            ...
            halfW = 1.0; minY = 1.15; maxY = 2.7; length = 0.3
        ...
        case .fullBarrier:
            ...
            halfW = 1.0; minY = 0; maxY = 1.9; length = 0.3
        ...
        case .train, .movingTrain:
            ...
            halfW = 1.15; minY = 0; maxY = 3.2; trainTopHeight = 3.0
            ...
            let frontLabel = Self.textNode(moving ? "AI SLOP EXPRESS" : "AI SLOP",
        ...
        case .flatbed:
            // low cargo car + ramp at the near (-z) end — run straight up
            length = customLength ?? 14
            rampZone = 4
            halfW = 1.15; minY = 0; maxY = 1.4; trainTopHeight = 1.4
            let deck = SCNBox(width: 2.4, height: 0.25, length: CGFloat(length), chamferRadius: 0.03)
            deck.materials = [Self.mat(UIColor(red: 0.4, green: 0.3, blue: 0.2, alpha: 1))]
            let d = SCNNode(geometry: deck)
            d.position = SCNVector3(0, 1.2, 0)
            node.addChildNode(d)
            let under = SCNBox(width: 2.2, height: 0.9, length: CGFloat(length * 0.95), chamferRadius: 0.05)
            under.materials = [Self.mat(UIColor(white: 0.15, alpha: 1))]
            let u = SCNNode(geometry: under)
            u.position = SCNVector3(0, 0.65, 0)
            node.addChildNode(u)
            // ramp at -z end
            let ramp = SCNBox(width: 2.4, height: 0.1, length: CGFloat(rampZone), chamferRadius: 0.02)
            ramp.materials = [Self.mat(UIColor(white: 0.5, alpha: 1), roughness: 0.4, metal: 0.5)]
            let rp = SCNNode(geometry: ramp)
            rp.position = SCNVector3(0, 0.7, -length / 2 + rampZone / 2)
            rp.rotation = SCNVector4(1, 0, 0, -0.35)
